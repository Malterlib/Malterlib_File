	...
	namespace NFile
	{
		...
		public:
			...
			public:
				...
				DMibIntrusiveLink(CFile, NIntrusive::TCAVLLink<>, m_FileClusterIDLink);
				...
				DMibIntrusiveLink(CFile, NIntrusive::TCAVLLink<>, m_DirectoryIDLink);
			...
			NIntrusive::TCAVLTree<CFile::CLinkTraits_m_DirectoryIDLink, CFile::CCompareDirectoryID> m_Directories;
		};
		...
		void CVirtualFS::f_Open(NStream::CBinaryStream *_pStorageStream)
		{
			...
			mp_ClusterIDsPerCluster = (mp_RootData.m_ClusterSize - (sizeof(NDataProcessing::CHashDigest_MD5) + sizeof(CClusterID)*2 + sizeof(uint64))) / sizeof(CClusterID);
		...
		CVirtualFS::ECheckFSError CVirtualFS::fs_CheckFS(NStream::CBinaryStream *_pStorageStream, CCheckReporter *_pReporter, bint _bFix, CVirtualFS *_pFixDestinationFS)
		{
			try
			{
				if (_pFixDestinationFS)
				{
					...
					uint64 ClusterIDsPerCluster = (ClusterSize - (sizeof(NDataProcessing::CHashDigest_MD5) + sizeof(CClusterID)*2 + sizeof(uint64))) / sizeof(CClusterID);
					uint64 nClusters = FileLenth / ClusterSize;
				...
				else
				{
					...
					for (int i = 0; i < 2; ++i)
					{
						...
						Context.mp_ClusterIDsPerCluster = (ClusterSize - (sizeof(NDataProcessing::CHashDigest_MD5) + sizeof(CClusterID)*2 + sizeof(uint64))) / sizeof(CClusterID);
		...
		bint CVirtualFS::CCheckFSContext::f_WriteFileData(CVirtualFS::CFileRecord &_FileRecord, NContainer::TCVector<uint8> &_Data, mint _Length)
		{
		...
		bint CVirtualFS::CCheckFSContext::fs_ReadFileData(uint32 _Version, uint64 _ClusterSize, uint64 _ClusterIDsPerCluster, NStream::CBinaryStream *_pStorageStream, CVirtualFS::CFileRecord &_FileRecord, NContainer::TCVector<uint8> &_Data)
		{
		...
		bint CVirtualFS::CCheckFSContext::f_ReadFileData(CVirtualFS::CFileRecord &_FileRecord, NContainer::TCVector<uint8> &_Data)
		{
			_Data.f_SetLen(_FileRecord.m_FileSize);
...
//				mp_pReporter->f_ReportError(NStr::CStr::CFormat("{}: Checking chain {} {}") << _FileName << Current << Chain.m_NextChainCluster);
			...
			{
				// Read directory data
				NContainer::TCVector<uint8> DirectoryData;
...
//				FileChainCluster = fp_GetNewCluster();
		...
		NDataProcessing::CHashDigest_MD5 CVirtualFS::f_GetFileChecksum(NStr::CStr const &_Path)
		{
			CFile File;
		...
		bint CVirtualFS::f_ReadFileToMemory(NStr::CStr const &_File, NContainer::TCVector<uint8> &_Data)
		{
			try
			{
