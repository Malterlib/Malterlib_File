...
namespace NMib
{
	namespace NFile
	{
		...
		NPtr::TCSharedPointer<NStream::CBinaryStreamDefaultRef> CFileSystemInterface_VirtualFS::f_OpenStream(const NStr::CStr &_FileName, NMib::NFile::EFileOpen _OpenFlags) const
		{
			NPtr::TCSharedPointer<NFile::TCBinaryStream_VirtualFSFile<NStream::CBinaryStreamDefaultRef>> pRet(fg_Construct());
		...
		NContainer::TCVector<uint8> CFileSystemInterface_VirtualFS::f_ReadFile(NStr::CStr const& _FileFrom) const
		{
			NContainer::TCVector<uint8> Ret;
